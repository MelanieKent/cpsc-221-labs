...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(w1Copy.begin(), w1Copy.end(), w1Copy.begin(), ::toupper);
    std::transform(w2Copy.begin(), w2Copy.end(), w2Copy.begin(), ::toupper);

    std::vector<std::string> pronunc1;
    std::vector<std::string> pronunc2;

    auto l1 = dict.find(w1Copy);
    auto l2 = dict.find(w2Copy);
